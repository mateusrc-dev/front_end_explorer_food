...
export function EditDesserts() {
  ...
  async function handleNewDish() {
    ...
    await api.post("/desserts", formData).then(() => { alert("Sobremesa cadastrada com sucesso"); navigate("/adm") }).catch(error => { if (error.response) { alert(error.response.data.message) } else { alert("Não foi possível cadastrar!") } })
      ...
      <main>
        ...
        <div className="imageAndNameDish">
          ...
          <div className="namedish">
            ...
            <input type="text" id="namedish" placeholder="Ex.: Bolo de morango" onChange={e => setName(e.target.value)} />
          </div>
        </div>
        ...
        <div className="textarea">
          <p>Descrição</p>
          <textarea placeholder="Fale brevemente sobre a sobremesa, seus ingredientes e composição" onChange={e => setDescription(e.target.value)} />
        </div>
